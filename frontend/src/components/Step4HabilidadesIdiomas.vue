<template>
      ...
      <div>
        ...
        <div class="mt-2 flex flex-wrap gap-2">
          <span
            ...
            :key="index"
            class="bg-teal-600 text-white px-3 py-1 rounded-full text-sm"
          >
            {{ h }}
          </span>
        </div>
  ...
  const adicionarHabilidade = (h = novaHabilidade.value) => {
    if (h && !curriculo.habilidades.includes(h)) curriculo.habilidades.push(h)
    novaHabilidade.value = ''
  }
  
  const adicionarIdioma = (i = novoIdioma.value) => {
    if (i && !curriculo.idiomas.includes(i)) curriculo.idiomas.push(i)
    novoIdioma.value = ''
  }
  </script>
